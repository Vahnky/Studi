...
copier tous les dossiers et fichiers contenu sur github dans le dossier créé ECF,
...
(
il y a une redondance du code pour que peu importe sur quelle page on commence et ce qu'on fasse, la BDD soit bien créée et la table nécessaire aussi, pour qu'il n'y ait pas d'erreurs,
et pour faire en sorte de suivre ce qui est demandé dans l'énoncé : ne pas utiliser phpmyadmin.
C'est pourquoi le php utilisé ici comme $pdo->exec("CREATE DATABASE IF NOT EXISTS xxxx") ... pour le déploiement du site sera différend : La BDD sera créée en amont avec un user Mysql et un mot de passe.
)
...
pour pouvoir ajouter les images des voitures à vendre : image principale et galerie d'image ; fonctionnalité disponible dans le compte employe, il faut bien prendre le dossier img sur github.
